 ...
 */
internal class PluginFileOperator(context: Context, private val inf: PluginInstallInfo) {
    ...
    val pluginOptimizedDirectory: File by lazy {
        val packageDir = File(pluginRootDir, "optimized").createDirIfNotExist()
        File(packageDir, inf.name)
    }


    /** 插件包路径*/
    val pluginPackageFile: File? by lazy {
        getPackageFile(pluginPackageDir)
    }

    fun getPackageFile(dir: File): File? {
        return dir.listFiles()?.singleOrNull { it.extension == "apk" }
    }
    ...
    /** 插件包锁路径 */
    val pluginLockFile: File by lazy {
        File(pluginPackageDir, "plugin.lock")
    }

    /** 插件包信息路径 */
    val pluginInfoFile: File by lazy {
        getPackageInfoJson(pluginPackageDir)
    }
    ...
    var pluginVersion: Int
        get() {
            val text = FileIOUtils.readFile2String(pluginLockFile) ?: "0"
            return text.toIntOrNull() ?: 0
        }
        set(value) {
            FileIOUtils.writeFileFromString(pluginLockFile, value.toString())
        }
